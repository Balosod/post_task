...
def create_user(user: schemas.UserCreateAndGet, db: Session = Depends(get_db)):
...
def create_user(user: schemas.UserCreateAndGet, db: Session = Depends(get_db),Authorize: AuthJWT = Depends()):
...
#ADD POUST ROUTE
@app.post("/addpost/")
def create_post_for_user(post: schemas.PostCreate, db: Session = Depends(get_db),Authorize: AuthJWT = Depends()):
    ...
    max_payload_size = 1 * 1024 * 1024  # 1 MB in bytes
    total_size = len(post.title.encode("utf-8")) + len(post.description.encode("utf-8"))
...
#DELETE POST ROUTE
@app.post("/deletpost/")
def delete_post(post: schemas.DeletePost,db: Session = Depends(get_db),Authorize: AuthJWT = Depends()):
